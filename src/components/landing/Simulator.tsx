...
export function Simulator() {
  ...
  const calcular = () => {
    ...
    const tarifaMedia = 0.75; // R$/kWh
    const custoWp = 4.5; // R$/Wp instalado
    const geracaoMedia = 130; // kWh/mês por kWp
    
    const potenciaNecessaria = consumoNum / geracaoMedia; // kWp
    const investimento = potenciaNecessaria * 1000 * custoWp;
    const economiaMensal = consumoNum * tarifaMedia * 0.92; // 92% de aproveitamento
              ...
              <AnimatePresence mode="wait">
                {resultado ? (
                  <motion.div
                    ...
                    exit={{ opacity: 0, x: -20 }}
                    className="bg-gradient-to-br from-primary to-solar-blue-light rounded-2xl p-6 text-primary-foreground"
                  >
